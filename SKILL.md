...
Step 3: 匹配 Alva 产品能力
  → 基于 KOL 的内容，从 Section 1 (Product Capabilities) 中选择
    最相关的 1-2 个能力作为 hook
  → 从 Section 6 (KOL Strategy) 中匹配合作形式：
...
| `{alva_capability_N}` | Section 1 | 匹配的 Alva 功能，用他们能理解的语言 |
...
## 9. Quick Reference: Common Growth Tasks
...
| "给这个 KOL 发 cold email" | 执行 Cold Email Automation 流程（调研 → 选模板 → 填充 → 确认 → 发送） |
| "批量给这批 KOL 发邮件" | 批量 Outreach 流程，逐个个性化，汇总审阅后发送 |
